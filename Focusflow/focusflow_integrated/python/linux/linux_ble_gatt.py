...
class _FocusFlowAdvertisement(ServiceInterface):
    ...
    (``UNO-Q-FF01``) and the configured discovery service UUID, so scanners
    on Windows / phones can find us either by name or by service match.

    The properties MUST be declared as ``@dbus_property`` (not just a
    plain ``get_properties()`` method) because BlueZ reads them via the
    standard ``org.freedesktop.DBus.Properties.GetAll`` call when
    building the GAP frame.  The patched ``GetManagedObjects`` handler
    in ``patch_dbus_fast.py`` honours the ``get_properties()`` method,
    but ``GetAll`` still uses the upstream ``@dbus_property`` machinery
    — without the decorators BlueZ sees an empty object and emits a
    zero-length advertisement, which is invisible to passive scanners.
    The wristband ``ble_server.Advertisement`` already follows this
    pattern; this class mirrors it.
    """

    def __init__(
        self, path: str, device_name: str, service_uuids: Sequence[str]
    ) -> None:
    ...
    # NOTE: signature strings must match the BlueZ introspection XML
    # exactly ("s" for UTF-8, "as" for array-of-string, "b" for bool,
    # "n" for uint16).  The patched GetManagedObjects also calls
    # ``get_properties()`` below, so the values returned by these
    # accessors are kept in lock-step with that helper.
    ...
    def TxPower(self, value: "n") -> None:
        # BlueZ writes back the TX power selected by the controller
        # when ``CanSetTxPower`` is supported; we keep the value for
        # debugging but never read it for routing decisions.
        self._tx_power = value  # type: ignore[attr-defined]

    def get_properties(self) -> Dict[str, Variant]:
        """Same data as the ``@dbus_property`` accessors above.

        Kept for compatibility with ``patch_dbus_fast``'s patched
        ``GetManagedObjects`` handler, which calls this method when it
        is present.

        NOTE: the ``@dbus_property`` accessors above are *descriptors*
        on the class, so ``self.Type`` returns the string ``"peripheral"``
        directly (NOT a callable).  Calling ``self.Type()`` would raise
        ``TypeError``.  Build the Variant dict from the raw values
        instead.
        """
        return {
    ...
    * ``stop`` tears down advertising, unregisters the application and
    ...
    def __init__(
        ...
        *,
        ...
        self.advertised_service_uuids = list(
            advertised_service_uuids or (SERVICE_UUID,)
        )
        if not self.advertised_service_uuids:
            raise ValueError("advertised_service_uuids must not be empty")
    ...
    async def _set_advertising(self, enable: bool) -> None:
        ...
        try:
            ...
            #
            # NOTE: ``Connectable`` is the *real* gatekeeper for incoming
            # BLE connections — if it is ``false`` (the default after a
            # fresh boot, or when the previous peripheral-mode app has
            # been torn down), passive scanners can still see the
            # advertisement but every CONNECT_REQ gets silently dropped
            # and the central sees ``Connection Failed``.  BlueZ does
            # NOT auto-set ``Connectable=true`` when an external
            # application registers a GATT app — we have to do it
            # ourselves.  See https://github.com/bluez/bluez/issues/220
            ...
            for prop, target in (
                ...
                try:
                    ...
                    self.logger.warning(
                        "Could not read %s on %s: %s",
                        prop, self.adapter_path, exc,
                    )
                    continue
                if current.value != target:
                    try:
                        ...
                        self.logger.info(
                            "Set adapter %s=%s on %s",
                            prop, target, self.adapter_path,
                        )
                    except DBusError as exc:
                        # Discoverable/Pairable/Connectable can be rejected
                        # when the agent isn't running; we still keep going
            ...
            self.logger.debug(
                "Adapter ready: alias=%s powered=Discoverable=Pairable=Connectable=true",
                self.device_name,
            )
        except DBusError as exc:
        ...
        # see ``UNO-Q-FF01`` + the configured discovery service UUID.
        ...
        try:
            ...
            self.logger.info(
                ...
                self.adapter_path, self.device_name, self.advertised_service_uuids,
            )
        except DBusError as exc:
            self._advertising_registered = False
            self.logger.error(
    ...
    async def start(self, loop: asyncio.AbstractEventLoop) -> None:
        ...
        * ``org.bluez.Error.Failed`` — ``RegisterApplication`` was
        ...
        self._loop = loop
        bus_address = os.environ.get("DBUS_SYSTEM_BUS_ADDRESS")
        if bus_address:
            self.logger.info(
                "connecting to system D-Bus via DBUS_SYSTEM_BUS_ADDRESS=%s",
                bus_address,
            )
        try:
            ...
            self.logger.error(
                ...
                "CAP_NET_ADMIN, e.g. via sudo). If running inside the "
                "Arduino App Lab container, start the host-side socat "
                "forwarder in host/start_dbus_proxy.sh and export "
                "DBUS_SYSTEM_BUS_ADDRESS accordingly.",
        ...
        try:
            ...
            self.logger.info(
                "Acquired system bus name 'com.focusflow' (reply=%s)",
                name_reply,
            )
        except DBusError as exc:
            self.logger.error(
        ...
        try:
            ...
            self.logger.error(
                "RegisterApplication(%s) failed: name=%r body=%r message=%s.  "
                "Common causes: (1) BlueZ already has another GATT "
                "application bound to the FocusFlow UUIDs - stop the other "
                "process and retry; (2) the controller is not "
                "advertising-capable; (3) the user lacks CAP_NET_ADMIN; "
                "(4) the patched dbus_fast handler returned an empty tree "
                "(see _focusflow_get_managed_objects).",
                self.APP_ROOT, getattr(exc, 'type', None),
                getattr(exc, 'body', None), exc,
            )
            raise
